...
class Config(object):
    def __init__(self, env_config_path, master_config_path, list_file_path, cli_tables, logger, global_ts, run_id, date_folder=None, main_path=None):
        ...
        if cli_tables:
            ...
            # Expect format: DB1|Schema1.Table1|Bad_Dist_Flag1|Splited_Slice1,DB2|Schema2.Table2|Bad_Dist_Flag2|Splited_Slice2
            tables = cli_tables.lower().split(',')
            for t in tables:
                try:
                    # Parse DB|Schema.Table|Flag
                    db_part, tbl_part, flg_bad_dist, total_slice = t.split('|')
                    ...
                    if db_part and sch_part and real_tbl:
                        self.execution_list.append({
                            ...
                            'big_tbl_n_slice': '0' if not total_slice else str(total_slice).strip()
                        })
                    else:
                        self.logger.warning("Invalid format in argument: {0}. Expected DB|Schema.Table|Bad Distributed Flg[Y,N]|Number of splited slices".format(t))
                        continue
            ...
            if os.path.exists(list_file_path) and os.path.getsize(list_file_path) > 0:
                self.logger.info("Expected format: DB|Schema.Table|Bad Distributed Flg[Y,N]|Number of splited slices")
                try:
                    with open(list_file_path, 'r') as f:
                        for line in f:
                            ...
                            try:
                                # Format: DB|Schema.Table|Bad_Dist_Flag|Big_Table_Flag
                                db_part, tbl_part, flg_bad_dist, total_slice = line.split('|')
                                sch_part, real_tbl = tbl_part.split('.')
        ...
        self.execution_list = list({(d['db'], d['schema'], d['table'], d['flg_bad_distributed'], d['big_tbl_n_slice']): d for d in self.execution_list}.values())
...
class QueryBuilder(object):
    ...
    def build_json_query(self, db, schema, table, categorized_cols, insert_logic_dict, ctas_schema, ctas_table, big_tbl_n_slice=None, idx_slice=None):
        try:
            ...
            if ctas_table:
                from_schema = ctas_schema
                from_tbl = ctas_table
            else:
                from_schema = schema
                from_tbl = table
                
            if big_tbl_n_slice: #fnedit#
                where_cond_slice_idx="WHERE ABS(hashtext(gp_segment_id::text || ctid::text)::bigint) % {0} = {1}".format(big_tbl_n_slice, idx_slice)
            else:
                where_cond_slice_idx=""
            ...
            sql = (
                ...
                "FROM \"{2}\".\"{3}\" "
                "{4};"
            ).format(full_table_name, metrics_sql, from_schema, from_tbl, where_cond_slice_idx)

            if big_tbl_n_slice: #fnedit#
                filename = "query_{0}_{1}_{2}_idxslice{3}_{4}.sql".format(db, schema, table, idx_slice, self.global_ts)
            else:
...
class Worker(threading.Thread):
    def __init__(self, thread_id, job_queue, config, builder, shell, json_merge, file_h, log_parser, tracker, logger, global_ts, status_file_filenm, status_file_locks, status_file_locks_lock):
    ...
    def run(self):
        ...
        while True:
            ...
            self.big_tbl_n_slice = task['big_tbl_n_slice'].strip()
            ...
            elif not str(self.big_tbl_n_slice).isdigit() or int(self.big_tbl_n_slice) < 0:
                self.status = "FAILED"
                self.error_message = "invalid input for Big Table Slices = {0}. Please define the value as Positive Integer.".format(self.big_tbl_n_slice)
                self.logger.error("Worker {0} Failed for {1}: {2}".format(self.name, full_name, self.error_message))
                self.tracker.add_result(full_name, self.status, self.error_message)
            else:
                for attempt in range(1, max_retries + 1):
                    ...
                    try:
                        ...
                        if missing_flag:
                            ...
                            if int(self.big_tbl_n_slice) > 0: #fnedit#
                                # Big Table : Need to split to slices
                                for idx_slice in range(int(self.big_tbl_n_slice)):
                                    sql_file = self.builder.build_json_query(self.db, self.schema, table, cat_cols, insert_logic_dict, self.config.ctas_schema, self.ctas_table, big_tbl_n_slice=self.big_tbl_n_slice, idx_slice=idx_slice)
                                    output_filename = "gp_{0}_{1}_{2}_idxslice{3}_{4}.json".format(self.db, self.schema, table, idx_slice, self.global_ts)
                                ...
                                # Calculate and Merge all Json files
                                idx_file_pattern = "gp_{0}_{1}_{2}_idxslice*_{3}.json".format(self.db, self.schema, table, self.global_ts)
                                final_output_filename = "gp_{0}_{1}_{2}_{3}.json".format(self.db, self.schema, table, self.global_ts)
                                final_local_path = os.path.join(self.config.local_temp_dir, final_output_filename)
                                self.json_merge.merge_json_slices(idx_file_pattern, final_output_filename)
                                
                                json_output_dir = os.path.join(self.config.nas_dest_base, self.db, self.schema)
                                self.json_output_path_file = os.path.join(json_output_dir, final_output_filename)
                                self.file_h.copy_to_nas(final_local_path, json_output_dir)
...
class JsonMerge(object):
    def __init__(self, logger, local_temp_dir, dttm_type):
        self.logger = logger
        self.local_temp_dir = local_temp_dir
        self.datetime_dtypes = dttm_type
        
        self.agg_keys = {"count", "sum", "min", "max", "min_md5", "max_md5"}
        self.re_date = re.compile(r"^(?P<y>\d+)-(?P<m>0[1-9]|1[0-2])-(?P<d>0[1-9]|[12][0-9]|3[01])(?:\s+(?P<era>BC|AD))?$")
        self.re_time = re.compile(
            r"^(?P<h>(?:[01][0-9]|2[0-3]|24)):(?P<mi>[0-5][0-9]):(?P<s>[0-5][0-9])"
            r"(?:\.(?P<f>\d+))?"
            r"(?P<tz>Z|[+-]\d{2}(?::?\d{2})?)?$"
        )
        self.re_ts = re.compile(
            r"^(?P<y>\d+)-(?P<m>0[1-9]|1[0-2])-(?P<d>0[1-9]|[12][0-9]|3[01])[ T]"
            r"(?P<h>(?:[01][0-9]|2[0-3]|24)):(?P<mi>[0-5][0-9]):(?P<s>[0-5][0-9])"
            r"(?:\.(?P<f>\d+))?"
            r"(?P<tz>Z|[+-]\d{2}(?::?\d{2})?)?"
            r"(?:\s+(?P<era>BC|AD))?$"
    ...
    def parse_tz_seconds(self, tz):
        ...
        if ":" in tz_body:
            hh, mm = tz_body.split(":")
        elif len(tz_body) == 4:  # e.g. 0700
            hh, mm = tz_body[:2], tz_body[2:]
        else:  # e.g. 07
            hh, mm = tz_body, "00"

        return sign * (int(hh) * 3600 + int(mm) * 60)

    def synthetic_day_index(self, year, month, day):
        return year * 372 + month * 31 + day

    def normalize_year(self, y, era):
        y = int(y)
        if era == "BC":
            return -(y - 1)
        else:
    ...
    def parse_temporal(self, value):
        s = str(value).strip()

        # TIMESTAMP
        m = self.fullmatch(self.re_ts, s)
        if m:
            year = self.normalize_year(m.group("y"), m.group("era"))
            mo = int(m.group("m"))
            d = int(m.group("d"))
            h = int(m.group("h"))
            mi = int(m.group("mi"))
            ...
            local_us = (h * 3600 + mi * 60 + se) * 1000000 + frac_us
            offset = self.parse_tz_seconds(tz) * 1000000

            utc_us = local_us - offset
            day_shift, sec_us = divmod(utc_us, 86400000000)

            return ("ts", self.synthetic_day_index(year, mo, d) + day_shift, sec_us)

        # DATE
        m = self.fullmatch(self.re_date, s)
        if m:
            year = self.normalize_year(m.group("y"), m.group("era"))
            mo = int(m.group("m"))
            d = int(m.group("d"))
            return ("date", self.synthetic_day_index(year, mo, d))

        # TIME
        m = self.fullmatch(self.re_time, s)
        if m:
            h = int(m.group("h"))
            mi = int(m.group("mi"))
            se = int(m.group("s"))
            ...
            return ("time", day_shift, sec_us)

        return None

    def compare_key(self, value, dtype=None):
        s = str(value)

        # 1. Datetime
        if dtype in self.datetime_dtypes:
            t = self.parse_temporal(s)
            if t:
                return t
    ...
    def ensure_same_type(self, values, dtype=None):
        types = [self.compare_key(v, dtype)[0] for v in values if v is not None]
        if len(set(types)) > 1:
            raise ValueError("Mixed types in comparison: {0}".format(values))

    def calc_min(self, values, dtype=None):
        values = [v for v in values if v is not None]
        if not values:
            return None
        self.ensure_same_type(values, dtype)
        return min(values, key=lambda v: self.compare_key(v, dtype))

    def calc_max(self, values, dtype=None):
        values = [v for v in values if v is not None]
        if not values:
            return None
        self.ensure_same_type(values, dtype)
        return max(values, key=lambda v: self.compare_key(v, dtype))

    def is_leaf(self, d):
        if not isinstance(d, dict):
            return False

        if not any(k in d for k in self.agg_keys):
            return False

        return not any(isinstance(v, (dict, list)) for v in d.values())

    # merge content in methods
    def merge_leaf(self, dicts):
        out = OrderedDict()
        v_dtype = None

        if "data_type" in dicts[0]:
            v_dtype = dicts[0]["data_type"]
            out["data_type"] = v_dtype

        # sum
        sums = [d.get("sum") for d in dicts if d.get("sum") is not None]
        if sums:
            out["sum"] = str(sum(Decimal(str(s)) for s in sums))
        elif any("sum" in d for d in dicts):
            out["sum"] = None

        # min
        mins = [d.get("min") for d in dicts if d.get("min") is not None]
        if mins:
            out["min"] = self.calc_min(mins, v_dtype)
        elif any("min" in d for d in dicts):
            out["min"] = None

        # max
        maxs = [d.get("max") for d in dicts if d.get("max") is not None]
        if maxs:
            out["max"] = self.calc_max(maxs, v_dtype)
        elif any("max" in d for d in dicts):
            out["max"] = None

        # md5
        min_md5 = [d.get("min_md5") for d in dicts if d.get("min_md5") is not None]
        if min_md5:
            out["min_md5"] = min(min_md5)
        elif any("min_md5" in d for d in dicts):
            out["min_md5"] = None

        max_md5 = [d.get("max_md5") for d in dicts if d.get("max_md5") is not None]
        if max_md5:
            out["max_md5"] = max(max_md5)
        elif any("max_md5" in d for d in dicts):
            out["max_md5"] = None
    ...
    def merge_nodes(self, nodes):
        ...
        # preserve key order from first node
        ...
        for k in keys:
            ...
            if k == "count":
                ...
                if counts:
                    if len(counts) != len(values):
                        raise ValueError("Inconsistent count values: {0}".format(values))

                    out["count"] = sum(int(c) for c in counts)
                else:
                    out["count"] = None
                continue
            ...
            if all(isinstance(v, dict) for v in values if v is not None):
                if values and self.is_leaf(values[0]):
                    out[k] = self.merge_leaf(values)
                else:
                    out[k] = self.merge_nodes(values)
            else:
                out[k] = values[0]
        return out

    def merge_json_slices(self, file_pattern, output_filename):
        files = glob.glob("{0}/{1}".format(self.local_temp_dir, file_pattern))
        data = []
        if files:
            for f in files:
                self.logger.info("Loading: {0}".format(f))
                with open(f) as fh:
                    data.append(json.load(fh, object_pairs_hook=OrderedDict))
                    
            try:
                result = self.merge_nodes(data)
                output_path = "{0}/{1}".format(self.local_temp_dir, output_filename)
                with open(output_path, 'w') as f:
                    json.dump(result, f, ensure_ascii=False)
                    f.write('\n')
            
            except Exception as e:
                err_msg = "Error merging JSON files of all slices: {0}".format(e)
                self.logger.error(err_msg)
                raise RuntimeError(err_msg)
        else:
            err_msg = "Not found any json files of all slices (pattern={0}/{1})".format(self.local_temp_dir, file_pattern)
            self.logger.error(err_msg)
            raise RuntimeError(err_msg)
...
class GreenplumExportJob(object):
    def __init__(self, args, logger, log_path, global_date_folder, global_ts, main_path):
        ...
        self.json_merge = JsonMerge(logger, self.config.local_temp_dir, self.config.type_mapping.get("MIN_MAX", []))
    ...
    def run(self):
        ...
        for i in range(num_workers):
            w = Worker(i+1, self.job_queue, self.config, self.builder, self.shell, self.json_merge, self.file_h, self.log_parser, self.tracker, self.logger, self.global_ts, status_file_filenm, self.status_file_locks, self.status_file_locks_lock)
            workers.append(w)
            w.start()
